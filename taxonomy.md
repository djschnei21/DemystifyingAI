...
Anthropomorphic metaphors have their place in how we talk about agentic systems. Treating an agent like a *persona* or a subagent like a *teammate* can speed up early design conversations. Even *skill* quietly frames a collection of conditionally loaded text files as a human competency. We __love__ to personify these things. However, once a conversation shifts from high-level product ideation to deep systems design, this vocabulary becomes too imprecise. To build predictable, scalable production systems, we have to set the metaphors aside long enough to look at what the machine is actually doing.
