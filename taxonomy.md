...
Anthropomorphic metaphors have their place in how we talk about agentic systems. Treating an agent like a *persona* or a subagent like a *teammate* can speed up early design conversations. Even the word *skill* quietly frames a collection of conditionally loaded text files as a human competency. We __love__ to personify these things, and for high-level ideation, it works. But personification has a strict ceiling. Once a conversation shifts to deep systems design, this vocabulary becomes too imprecise. You cannot build a system's foundation on a metaphor. To bring true architectural thinking to these systems, we have to look past the human analogies and understand the actual, mechanical differences between the constructs we are assembling.

This document is what you get when you strip away the vibes and magic. It is a functional explanation of the components that make up modern agentic systems, from the model request boundary to the harness patterns built around it. The thesis is simple: agentic architecture is harness architecture, and harness architecture is context architecture. Once you ask what the model can see, what the harness can execute, and what happens to context between model requests, the tradeoffs stop being guesswork and become things you can reason about directly.
