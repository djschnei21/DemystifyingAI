...
Anthropomorphic metaphors have their place in how we talk about agentic systems. Calling an agent a *persona* or a subagent a *teammate* can speed up early design conversations. Even *skill*, now an industry-standard term, quietly turns a collection of conditionally-loaded text files into something that sounds like a competency. However, for some engineers this vocabulary can prove inaccurate or, worse, inaccessible. For those engineers, myself included, it's worth setting the metaphors aside long enough to see what each component actually *does*.
